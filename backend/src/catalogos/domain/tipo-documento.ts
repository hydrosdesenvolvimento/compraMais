 ...
 * (Balanço do exercício — RN006) e `categoria` (retenção legal por categoria — RN015/UC017).
 */
export class TipoDocumento extends ItemCatalogo {
  ...
  static criar(input: {
    ...
    exigeValidade?: boolean; exigeExercicio?: boolean; userName?: string;
  }): TipoDocumento {
  ...
  static deEstado(s: TipoDocumentoState): TipoDocumento {
    return new TipoDocumento(s.meta, s.nome, s.formato, s.categoria, s.exigeValidade, s.exigeExercicio, s.situacao);
  }
  ...
  estado(): TipoDocumentoState {
    return {
      ...
      exigeValidade: this._exigeValidade, exigeExercicio: this._exigeExercicio, situacao: this._situacao,
    };
  }
  ...
  editar(campos: Partial<{ nome: string; formato: string; categoria: string; exigeValidade: boolean; exigeExercicio: boolean }>, userName: string): CampoDiff[] {
