...
export function registrarRotasMalote(app: FastifyInstance, deps: { gerar: GerarMalote }): void {
  app.post('/malotes', async (req, reply) => {
    if (!cpl(req)) return reply.code(403).send({ codigo: 'RBAC', mensagem: 'Apenas CPL/Administrador gera malote.' });
  ...
  app.get('/malotes/:id', async (req, reply) => {
    if (!cpl(req)) return reply.code(403).send({ codigo: 'RBAC', mensagem: 'Acesso restrito.' });
    const { id } = req.params as { id: string };
    const m = (await deps.gerar.consultar({})).find((x) => x.id === id);
    if (!m) return reply.code(404).send({ codigo: 'MaloteNaoEncontrado', mensagem: 'Malote não encontrado.' });
  ...
  app.post('/malotes/:id/exportar', async (req, reply) => {
    if (!cpl(req)) return reply.code(403).send({ codigo: 'RBAC', mensagem: 'Apenas CPL/Administrador exporta.' });
    const { id } = req.params as { id: string };
    try {
