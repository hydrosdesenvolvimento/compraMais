 ...
 * Exercita o fluxo real: cadastro → login (obtém o userId = id da ContaAcesso do titular) →
...
async function cadastrarESentar(app: Awaited<ReturnType<typeof buildServer>>) {
  ...
  return { fornecedorId, titularUserId };
}
...
describe('Rotas /fornecedores/:id/procuradores (UC019)', () => {
  it('fluxo do titular: convidar → listar → remover (remoção lógica)', async () => {
    ...
    const h = { 'x-user-id': titularUserId };
  ...
  it('procurador NÃO convida outro procurador → 403 (UC019 exceção / RN010)', async () => {
    const app = await buildServer();
    const { fornecedorId, titularUserId } = await cadastrarESentar(app);
    const convite = await app.inject({ method: 'POST', url: `/fornecedores/${fornecedorId}/procuradores`, headers: { 'x-user-id': titularUserId }, payload: { identificador: 'proc@empresa.com' } });
    const procuradorContaId = convite.json().procuradorContaId as string;

    const res = await app.inject({ method: 'POST', url: `/fornecedores/${fornecedorId}/procuradores`, headers: { 'x-user-id': procuradorContaId }, payload: { identificador: 'outro@empresa.com' } });
  ...
  it('ator sem vínculo na empresa → 404', async () => {
    ...
    const res = await app.inject({ method: 'POST', url: `/fornecedores/${fornecedorId}/procuradores`, headers: { 'x-user-id': 'estranho' }, payload: { identificador: 'x@empresa.com' } });
