...
export function AppShell({
  menu, usuario, children, rodape = 'Versão 2.0 · MVP FIEAC',
    ...
    <div className="cm-shell" data-cy="app-shell">
        ...
        <div className="cm-brandrow">
          <span className="cm-brandfull">
            ...
            <span className="cm-brand-text">
              <span className="cm-brand-name">Compra Mais</span>
              <span className="cm-brand-sub">RIO BRANCO</span>
            </span>
          </span>
          <span className="cm-brandmini"><IconePredio width={22} height={22} /></span>
        </div>

        <nav className="cm-nav" aria-label="Menu principal">
          <div className="cm-grouplabel">MENU</div>
            ...
            {menu.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="cm-navlabel">{m.rotulo}</span>
                </Link>
              );
            })}
          </div>
        </nav>
        <div className="cm-ver">{rodape}</div>
      </aside>
      ...
      <div className="cm-main" data-collapsed={recolhido ? '1' : '0'}>
        <header className="cm-topbar">
          <button className="cm-hamburger" onClick={alternarMenu} aria-label="Abrir menu"><IconeMenu /></button>
          ...
          <div style={{ position: 'relative' }}>
            <button className="cm-iconbtn" onClick={() => setNotifOpen((v) => !v)} aria-label="Notificações" data-cy="notificacoes">
              <IconeSino />
              ...
              <>
                <div className="cm-menu-pop" style={{ width: 340 }} role="region" aria-label="Notificações">
                  ...
                  <div style={{ padding: '11px 16px', borderTop: '1px solid var(--divider)', textAlign: 'center' }}>
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', font: '600 13px var(--font-body)', color: 'var(--azul-700)' }}>Ver todas as notificações</button>
                  </div>
                </div>
              </>
            )}
          ...
          <div style={{ position: 'relative' }}>
            <button className="cm-profile-btn" onClick={() => setProfileOpen((v) => !v)} data-cy="user-chip" aria-label="Menu do usuário">
