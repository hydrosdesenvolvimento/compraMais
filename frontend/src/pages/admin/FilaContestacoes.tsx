...
export function FilaContestacoes({ editalId }: { editalId: string }) {
  ...
  return (
    <div className="stack">
      <div><h1 className="page-title">{t('admin.contestacoes.titulo')}</h1><p className="page-sub">{t('admin.contestacoes.subtitulo')}</p></div>
      {itens.length === 0 && <p data-cy="sem-contestacoes">{t('admin.contestacoes.vazio')}</p>}
          ...
          <li key={c.id} data-cy="contestacao" className="card">
            ...
            {c.situacao === 'pendente' && (
              <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                <input data-cy="novo-cnae" className="input" style={{ maxWidth: 200 }} placeholder={t('admin.contestacoes.novoCnaePlaceholder')} value={novoCnae} onChange={(e) => setNovoCnae(e.target.value)} />
                <Botao data-cy="acatar" onClick={() => acatar.mutate(c.id)}>{t('admin.contestacoes.acatar')}</Botao>
                <input data-cy="motivo-recusa" className="input" style={{ maxWidth: 240 }} placeholder={t('admin.contestacoes.motivoPlaceholder')} value={motivo} onChange={(e) => setMotivo(e.target.value)} />
                <Botao data-cy="recusar" variante="secundario" onClick={() => recusar.mutate(c.id)} disabled={!motivo.trim() || recusar.isPending}>{t('admin.contestacoes.recusar')}</Botao>
              </div>
            )}
          </li>
        ))}
