                    ...
                    <tr key={s.id} data-cy="item-secretaria" data-id={s.id} data-ativo={s.ativo}>
                      ...
                      <td style={{ ...celula, textAlign: 'right' }}>
                        <div style={{ display: 'inline-flex', gap: 8, justifyContent: 'flex-end' }}>
                          <button type="button" data-cy="editar" title={t('admin.secretarias.acao.editar')} aria-label={t('admin.secretarias.acao.editar')} onClick={() => setModal({ modo: 'editar', item: s })} style={iconeAcao}>
                            <IconeLapis width={20} height={20} />
                          </button>
                          <button type="button" data-cy="alternar-situacao" title={t(`admin.secretarias.acao.${s.ativo ? 'inativar' : 'reativar'}`)} aria-label={t(`admin.secretarias.acao.${s.ativo ? 'inativar' : 'reativar'}`)} disabled={inativar.isPending || reativar.isPending} onClick={() => alternar(s)} style={{ ...iconeAcao, color: s.ativo ? 'var(--cinza-600, #556)' : 'var(--sucesso)' }}>
    ...
    <div style={overlay} onClick={onFechar} data-cy="modal-overlay">
      <div style={card} role="dialog" aria-modal="true" aria-label={titulo} data-cy="modal-secretaria" onClick={(e) => e.stopPropagation()}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, padding: '20px 24px', borderBottom: '1px solid var(--divider)' }}>
          ...
          <button type="button" onClick={onFechar} style={botaoX} data-cy="fechar-modal" aria-label={t('admin.secretarias.fechar')}><IconeFechar width={20} height={20} /></button>
        </header>
