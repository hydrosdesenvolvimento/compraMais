...
flowchart TD
  ...
  C -->|Não| E[Solicitar preench.<br/>manual]
  D --> F[Salvar perfil]
  E --> F
  F --> G[End]
```
...
flowchart TD
  ...
  E --> G{Aprovar?}
  G -->|Sim| H[Aprovar]
  G -->|Não| I[Reprovar<br/>com justif.]
...
A lógica interna da tarefa "Executar algoritmo" (`C` no diagrama) deve seguir estritamente a especificação detalhada do algoritmo iterativo, incluindo o tratamento de excedentes e arredondamento, conforme documentado na seção "Especificação do Algoritmo de Distribuição" do Caso de Uso `UC008 – Distribuir Demanda Equitativamente` no arquivo `06-CasosUsov2.md`.
