...
- `fornecedorId` (text \| nulo) — empresa representada (string opaca; ver migração 0003)
...
`idx_usuarios_email` e `idx_usuarios_google_id`. Aplicada no startup pelo migration runner
(`src/shared/db/migracoes.ts`), registrada em `schema_migrations`.
